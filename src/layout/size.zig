...
pub const Size = union(enum) {
    ...
    /// Proportion of whatever space remains after fixed slots are placed.
    ...
    /// Valid range is 0–100; values above 100 are caught at comptime when
    /// used inside a comptime blueprint (runtime paths use a debug assert).
    percent: u8,
};

test "Size: fixed holds its value" {
    const s = Size{ .fixed = 30 };
    try std.testing.expectEqual(@as(u16, 30), s.fixed);
}

test "Size: fraction holds its weight" {
    const s = Size{ .fraction = 2 };
    try std.testing.expectEqual(@as(u16, 2), s.fraction);
}

test "Size: percent holds its value" {
    const s = Size{ .percent = 25 };
    try std.testing.expectEqual(@as(u8, 25), s.percent);
}
